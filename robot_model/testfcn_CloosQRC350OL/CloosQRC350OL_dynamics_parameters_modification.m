...
function [m,rSges,Icges] = CloosQRC350OL_dynamics_parameters_modification(pkin,m,rSges,Icges)

%% Coder Information
%#codegen
%$cgargs {zeros(6,1),zeros(7,1),zeros(7,3),zeros(7,6)}
assert(isreal(pkin) && all(size(pkin) == [6 1]), ...
  'CloosQRC350OL_dynamics_parameters_modification: pkin has to be [6x1] (double)');
assert(isreal(m) && all(size(m) == [7 1]), ...
  'CloosQRC350OL_dynamics_parameters_modification: m has to be [7x1] (double)'); 
assert(isreal(rSges) && all(size(rSges) == [7,3]), ...
  'CloosQRC350OL_dynamics_parameters_modification: rSges has to be [7x3] (double)');
assert(isreal(Icges) && all(size(Icges) == [7 6]), ...
  'CloosQRC350OL_dynamics_parameters_modification: Icges has to be [7x6] (double)'); 
