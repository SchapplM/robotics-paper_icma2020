...
% HybrDyn-Toolbox: CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp1.m
%
% Sources:
...
function [tau, v_i_i_ges, w_i_i_ges, f_i_i_ges, n_i_i_ges] = CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2(q, qD, qDD, phi_base, xD_base, xDD_base, ...
...
assert(isreal(q) && all(size(q) == [6 1]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: q has to be [6x1] (double)');
assert(isreal(qD) && all(size(qD) == [6 1]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: qD has to be [6x1] (double)');
assert(isreal(qDD) && all(size(qDD) == [6 1]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: qDD has to be [6x1] (double)');
assert(isreal(phi_base) && all(size(phi_base) == [3 1]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: phi_base has to be [3x1] (double)');
assert(isreal(xD_base) && all(size(xD_base) == [6 1]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: xD_base has to be [6x1] (double)');
assert(isreal(xDD_base) && all(size(xDD_base) == [6 1]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: xDD_base has to be [6x1] (double)');
assert(isreal(pkin) && all(size(pkin) == [6 1]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: Kinematic parameters pkin have to be [6x1] (double)');
assert(isreal(m_num) && all(size(m_num) == [7 1]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: m_num has to be [7x1] (double)');
assert(isreal(mrSges_num_mdh) && all(size(mrSges_num_mdh) == [7,3]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: mrSges_num_mdh has to be [7x3] (double)');
assert(isreal(Ifges_num_mdh) && all(size(Ifges_num_mdh) == [7 6]), ...
  'CloosQRC350OL_invdyn_floatb_eulxyz_nnew_vp2: Ifges_num_mdh has to be [7x6] (double)');
